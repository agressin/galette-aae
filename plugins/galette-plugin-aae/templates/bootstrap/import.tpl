        <p class="center">
            <a class="button" id="preferences" href="{$galette_base_path}import_model.php">{_T string="Configure import model"}</a>
        </p>
...
{if $cycles|@count gt 0}
				<select name="selected_cycle">
	{foreach $cycles as $id_cycle => $nom}
					<option value={$id_cycle} {if isset($selected_cycle) and $selected_cycle eq $id_cycle} selected{/if}>{$nom}</option>
	{/foreach}
				</select>
{/if}
