...
{if $login->isStaff() or $login->isAdmin()}
    ...
    <li{if $PAGENAME eq "gestion_cycles.php"} class="selected"{/if}>
    <a href="{$galette_base_path}{$aaetools_path}gestion_cycles.php" title="{_T string="Cycles managment"}">{_T string="Cycles managment"}</a>
    </li>
{/if}
